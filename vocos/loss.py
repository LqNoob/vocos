...
class MelSpecReconstructionLoss(nn.Module):
    ...
    def forward(self, y_hat, y) -> torch.Tensor:
        ...
        if self.padding == "same":
            ...
            y_hat = torch.nn.functional.pad(y_hat, (pad // 2, pad // 2), mode="reflect")
            y = torch.nn.functional.pad(y, (pad // 2, pad // 2), mode="reflect")

        mel_hat = safe_log(self.mel_spec(y_hat), clip_val=1e-5)
        mel = safe_log(self.mel_spec(y), clip_val=1e-5)
