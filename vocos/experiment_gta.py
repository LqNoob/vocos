...
class VocosExp(pl.LightningModule):
    ...
    def load_pretrained_model(self):
        self.state_dict_all = torch.load(self.pretrained_model, map_location="cuda")
        self.load_state_dict(self.state_dict_all['state_dict'])
    ...
    def configure_optimizers(self):
        ...
        # TODO
        opt_disc.load_state_dict(self.state_dict_all['optimizer_states'][0])
        opt_gen.load_state_dict(self.state_dict_all['optimizer_states'][-1])
    ...
    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        audio_input, acoustic_mel = batch

        # train discriminator
        if optimizer_idx == 0 and self.train_discriminator:
            with torch.no_grad():
                audio_hat = self(acoustic_mel, **kwargs)
