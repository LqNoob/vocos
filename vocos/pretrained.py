...
class Vocos(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, model_path: str, config_path: str, repo_id: str = None, revision: Optional[str] = None) -> Vocos:
        """
        ...
        if isinstance(model.feature_extractor, EncodecFeatures):
            ...
            for key, value in state_dict.items():
                if key.split('.')[0] in ['feature_extractor', 'backbone', 'head']:
                    state_dict_processed[key] = value

        model.load_state_dict(state_dict_processed)
        model.eval()
        return model
